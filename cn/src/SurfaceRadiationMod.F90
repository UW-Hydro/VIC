...
!
! !INTERFACE:
   subroutine SurfaceRadiation(year, month, day, secs, dtime, lbp, ubp)
!
! !DESCRIPTION: 
...
! !ARGUMENTS:
     ...
     integer, intent(in)  :: year,month,day,secs !  calendar info for current time step
...
! 08/19/13, Michael Brunke: Adapted for use in VIC
...
!------------------------------------------------------------------------------
     ...
     slasun        => clm3%g%c%p%pps%slasun
     slasha        => clm3%g%c%p%pps%slasha
     ...
     sabg          => clm3%g%c%p%pef%sabg
     sabv          => clm3%g%c%p%pef%sabv
     snowdp        => clm3%g%c%cps%snowdp
     fsa           => clm3%g%c%p%pef%fsa
     fsa_r         => clm3%g%c%p%pef%fsa_r
     fsr           => clm3%g%c%p%pef%fsr
     parsun        => clm3%g%c%p%pef%parsun
     parsha        => clm3%g%c%p%pef%parsha
     fsds_vis_d    => clm3%g%c%p%pef%fsds_vis_d
     fsds_nir_d    => clm3%g%c%p%pef%fsds_nir_d
     fsds_vis_i    => clm3%g%c%p%pef%fsds_vis_i
     fsds_nir_i    => clm3%g%c%p%pef%fsds_nir_i
     fsr_vis_d     => clm3%g%c%p%pef%fsr_vis_d
     fsr_nir_d     => clm3%g%c%p%pef%fsr_nir_d
     fsr_vis_i     => clm3%g%c%p%pef%fsr_vis_i
     fsr_nir_i     => clm3%g%c%p%pef%fsr_nir_i
     fsds_vis_d_ln => clm3%g%c%p%pef%fsds_vis_d_ln
     fsds_nir_d_ln => clm3%g%c%p%pef%fsds_nir_d_ln
     fsr_vis_d_ln  => clm3%g%c%p%pef%fsr_vis_d_ln
     fsr_nir_d_ln  => clm3%g%c%p%pef%fsr_nir_d_ln
     eff_kid       => clm3%g%c%p%pps%eff_kid
     eff_kii       => clm3%g%c%p%pps%eff_kii
     sun_faid      => clm3%g%c%p%pps%sun_faid
     sun_faii      => clm3%g%c%p%pps%sun_faii
     sha_faid      => clm3%g%c%p%pps%sha_faid
     sha_faii      => clm3%g%c%p%pps%sha_faii
     sun_add       => clm3%g%c%p%pef%sun_add
     tot_aid       => clm3%g%c%p%pef%tot_aid
     sun_aid       => clm3%g%c%p%pef%sun_aid
     sun_aii       => clm3%g%c%p%pef%sun_aii
     sha_aid       => clm3%g%c%p%pef%sha_aid
     sha_aii       => clm3%g%c%p%pef%sha_aii
     sun_atot      => clm3%g%c%p%pef%sun_atot
     sha_atot      => clm3%g%c%p%pef%sha_atot
     sun_alf       => clm3%g%c%p%pef%sun_alf
     sha_alf       => clm3%g%c%p%pef%sha_alf
     sun_aperlai   => clm3%g%c%p%pef%sun_aperlai
     sha_aperlai   => clm3%g%c%p%pef%sha_aperlai
     ...
     sabg_lyr         => clm3%g%c%p%pef%sabg_lyr
     snl              => clm3%g%c%cps%snl
     sfc_frc_aer      => clm3%g%c%p%pef%sfc_frc_aer
     sfc_frc_aer_sno  => clm3%g%c%p%pef%sfc_frc_aer_sno
     albgrd_pur       => clm3%g%c%cps%albgrd_pur
     albgri_pur       => clm3%g%c%cps%albgri_pur
     sfc_frc_bc       => clm3%g%c%p%pef%sfc_frc_bc
     sfc_frc_bc_sno   => clm3%g%c%p%pef%sfc_frc_bc_sno
     albgrd_bc        => clm3%g%c%cps%albgrd_bc
     albgri_bc        => clm3%g%c%cps%albgri_bc
     sfc_frc_oc       => clm3%g%c%p%pef%sfc_frc_oc
     sfc_frc_oc_sno   => clm3%g%c%p%pef%sfc_frc_oc_sno
     albgrd_oc        => clm3%g%c%cps%albgrd_oc
     albgri_oc        => clm3%g%c%cps%albgri_oc
     sfc_frc_dst      => clm3%g%c%p%pef%sfc_frc_dst
     sfc_frc_dst_sno  => clm3%g%c%p%pef%sfc_frc_dst_sno
     ...
     fsr_sno_vd       => clm3%g%c%p%pef%fsr_sno_vd
     fsr_sno_nd       => clm3%g%c%p%pef%fsr_sno_nd
     fsr_sno_vi       => clm3%g%c%p%pef%fsr_sno_vi
     fsr_sno_ni       => clm3%g%c%p%pef%fsr_sno_ni
     fsds_sno_vd      => clm3%g%c%p%pef%fsds_sno_vd
     fsds_sno_nd      => clm3%g%c%p%pef%fsds_sno_nd
     fsds_sno_vi      => clm3%g%c%p%pef%fsds_sno_vi
     fsds_sno_ni      => clm3%g%c%p%pef%fsds_sno_ni
...
!                 print *, p, ivt(p), laisun(p), elai(p), fsun(p)
...
!#endif
     ...
     ! Probably not needed as VIC does radiation, MAB 8/18/13
!     do p = lbp, ubp
