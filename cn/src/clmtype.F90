...
! Revised by Michael A. Brunke for use in VIC (10/2/2012)
!
!*******************************************************************************
!----------------------------------------------------
...
type, public :: gridcell_type
   ...
   type(column_type) :: c             !column structure

   ! g/l/c/p hierarchy, local g/l/c/p cells only
   integer, pointer :: gridcell(:)      !index into gridcell level quantities
   integer, pointer :: wgcell(:)        !weight (relative to gridcell)
   integer, pointer :: coli(:)          !beginning column index
   integer, pointer :: colf(:)          !ending column index
   integer, pointer :: ncolumns(:)      !number of columns
   integer, pointer :: pfti(:)          !beginning pft index for each column
   integer, pointer :: pftf(:)          !ending pft index for each column
   integer, pointer :: npfts(:)         !number of pfts for each column
...
!#endif
   ...
   type(gridcell_estate_type) :: ges    !average of energy states all columns
   type(gridcell_wstate_type) :: gws    !average of water states all columns
   type(gridcell_cstate_type) :: gcs    !average of carbon states all columns
   type(gridcell_nstate_type) :: gns    !average of nitrogen states all cols
   type(gridcell_vstate_type) :: gvs    !average of VOC states all columns
   type(gridcell_efstate_type):: gve	!gridcell VOC emission factors
   
   ! flux variables defined at the gridcell level
   type(gridcell_wflux_type) :: gwf     !average of water fluxes all columns
   type(gridcell_cflux_type) :: gcf     !average of carbon fluxes all columns
   type(gridcell_nflux_type) :: gnf     !average of nitrogen fluxes all cols
   type(gridcell_vflux_type) :: gvf     !average of VOC fluxes all columns
...
!----------------------------------------------------
  type, public :: atm2lnd_type
     ...
     real(r8), pointer :: forc_pco2(:)    !C02 partial pressure (Pa)
...
!#endif
  ...
  type(atm2lnd_type),public,target, save :: clm_a2l  ! a2l fields on atm grid
