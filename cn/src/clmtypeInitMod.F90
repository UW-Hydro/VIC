...
!    integer :: nump         ! total number of pfts across all processors
    ...
    real(r8) :: pftfrac(0:mxpft)      ! CLM PFT fractions
...
!      end do
      if(vegtype(p) .eq. 0) then
!        m = 2
        m = 1
        pftfrac(m) = pftfrac(m) + vegfrac(p)
        npfts = npfts + 1
      else if(vegtype(p) .eq. 1) then
        m = 5
        pftfrac(m) = pftfrac(m) + vegfrac(p)
        npfts = npfts + 1
      else if(vegtype(p) .eq. 2) then
        m = 3
        pftfrac(m) = pftfrac(m) + vegfrac(p)
        npfts = npfts + 1
      else if(vegtype(p) .eq. 3) then
!        m = 8
        m = 7
        pftfrac(m) = pftfrac(m) + vegfrac(p)
        npfts = npfts + 1
      else if(vegtype(p) .eq. 4) then
!        m = 2
        m = 1
        if(pftfrac(m) .eq. 0.) then
          npfts = npfts + 1
        end if
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.5
...
!        m = 8
        ...
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.5
      else if(vegtype(p) .eq. 5) then
!        m = 2
        m = 1
        if(pftfrac(m) .eq. 0.) then
          npfts = npfts + 1
        end if
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.4
...
!        m = 8
        ...
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.1
        ...
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.3

        m = 12
        if(pftfrac(m) .eq. 0.) then
          npfts = npfts + 1
        end if
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.2
...
!        m = 8
        ...
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.05

        m = 13
        if(pftfrac(m) .eq. 0.) then
          npfts = npfts + 1
        end if
        pftfrac(m) = pftfrac(m) + vegfrac(p) * 0.15
...
!        npfts = npfts + 1
!      end if
      pftsum = pftsum + pftfrac(m)
    enddo

    if(pftsum > 1.) then
      do m = 0, mxpft
        pftfrac(m) = pftfrac(m) / pftsum
      enddo
      pftsum = 1.
    end if
    ...
    do c = begc, endc
      ...
      do m = 0, mxpft
        if(pftfrac(m) .gt. 0.) then
          clm3%g%c%p%gridcell((c-1)*npfts + p) = 1
          clm3%g%c%p%wtgcell((c-1)*npfts + p) = pftfrac(m)
          clm3%g%c%p%column((c-1)*npfts + p) = c
          clm3%g%c%p%wtcol((c-1)*npfts + p) = pftfrac(m)
          clm3%g%c%p%itype((c-1)*npfts + p) = m
          clm3%g%c%p%mxy((c-1)*npfts + p) = m
          p = p + 1
        endif
      enddo
    enddo
    ...
    do c = begc, endc
      ...
      clm3%g%c%pfti(c) = 1
      clm3%g%c%pftf(c) = npfts
      clm3%g%c%npfts(c) = npfts
      clm3%g%c%itype(c) = 1
    enddo
