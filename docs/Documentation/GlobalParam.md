...
The order of the options in the global parameter file is not important, but the complete option name must be followed by the required option type information. To help in understanding this file, an [example file](#ExampleGlobalParameterFile) has been attached at the bottom of this page.

### Sections of Global Parameter File

*   [Define Simulation Parameters](#DefineSimulationParameters)
*   [Main Simulation Parameters](#DefineMainSimulationParams)
*   [Energy Balance Parameters](#DefineEnergyBalanceParams)
*   [Soil Temperature Parameters](#DefineSoilTempParams)
*   [Precipitation Parameters](#DefinePrecipParams)
*   [Turbulent Flux Parameters](#DefineFluxParams)
*   [Met. Forcing Disaggregation Parameters](#DefineDisaggParams)
*   [Define Carbon Parameters](#DefineCarbonParams)
*   [Miscellaneous Parameters](#DefineMiscParams)
    *   [Define State Files](#DefineStateFiles)
    *   [Define Meteorological Forcing Files](#DefineForcingFiles)
    *   [Define Parameter Files](#DefineParameterFiles)
    *   [Define Lake Parameters](#DefineLakeParams)
    *   [Define Output Files](#DefineOutputFiles)
    *   [Example Global Parameter File](#ExampleGlobalParameterFile)

### Other Information

*   [Obsolete Options from Earlier Versions](#ObsoleteOptions)
*   [Note about compatibility with previous versions of VIC](../Development/CompatOpt.md)
...
| TIME_STEP     | integer   | hours     | Simulation time step length (must divide 24 evenly). NOTE: TIME_STEP should be < 24 for FULL_ENERGY=TRUE or FROZEN_SOIL=TRUE.             |
| SNOW_STEP     | integer   | hours     | Length of time step used to solve the snow model (must divide 24 evenly; if TIME_STEP < 24, SNOW_STEP should = TIME_STEP)                 |
...
| QUICK_FLUX        | string        | TRUE or FALSE             | Option for computing the soil vertical temperature profile.  <li>**TRUE** = use the approximate method described by [Liang et al. (1999)](dx.doi.org/10.1029/94JD00483) to compute soil temperatures and ground heat flux; this method ignores water/ice phase changes. <li>**FALSE** = use the finite element method described in [Cherkauer and Lettenmaier (1999)](dx.doi.org/10.1029/1999JD900337) to compute soil temperatures and ground heat flux; this method is appropriate for accounting for water/ice phase changes.  <br><br>Default = FALSE (i.e. use [Cherkauer and Lettenmaier (1999)](dx.doi.org/10.1029/1999JD900337)) when running FROZEN_SOIL; and TRUE (i.e. use [Liang et al. (1999)](dx.doi.org/10.1029/94JD00483)) in all other cases. |
| IMPLICIT          | string        | TRUE or FALSE             | If TRUE the model will use an implicit solution for the soil heat flux equation of [Cherkauer and Lettenmaier (1999)](dx.doi.org/10.1029/1999JD900337) (QUICK_FLUX is FALSE), otherwise uses original explicit solution. When QUICK_FLUX is TRUE the implicit solution has no effect.  <br>The user can override this option by setting IMPLICIT to FALSE in the global parameter file. The implicit solution is guaranteed to be stable for all combinations of time step and thermal node spacing; the explicit solution is only stable for some combinations. If the user sets IMPLICIT to FALSE, VIC will check the time step, node spacing, and soil thermal properties to confirm stability. If the explicit solution will not be stable, VIC will exit with an error message. <br><br>Default = TRUE. |
| QUICK_SOLVE       | string        | TRUE or FALSE             | This option is a hybrid of QUICK_FLUX TRUE and FALSE. If TRUE model will use the method described by [Liang et al. (1999)](dx.doi.org/10.1029/94JD00483) to compute ground heat flux during the surface energy balance iterations, and then will use the method described in [Cherkauer and Lettenmaier (1999)](dx.doi.org/10.1029/1999JD900337) for the final solution step. <br><br>Default = FALSE.   |
...
| EXP_TRANS         | string        | TRUE or FALSE             | If TRUE the model will exponentially distributes the thermal nodes in the [Cherkauer and Lettenmaier (1999)](dx.doi.org/10.1029/1999JD900337) finite difference algorithm, otherwise uses linear distribution. (This is only used if FROZEN_SOIL = TRUE).  <br><br>Default = TRUE.   |
...
| Name            | Type   | Units         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|-----------------|--------|---------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| OUTPUT_FORCE    | string | TRUE or FALSE | Option to save VIC's internal, disaggregated forcings:  <li>**FALSE** = Run a simulation (including disaggregating the forcings).  <li>**TRUE** = Do not run a simulation; simply disaggregate the forcings and write them to output files. This allows us to use VIC as a meteorological forcing disaggregator. Note that in this mode of operation, VIC should be executed the same way as normal: VIC still must read a global parameter file that tells it the locations of the forcings and soil parameter file, and sets the model time step (this is the time step the forcings will be disaggregated to), the output time step (usually the same as the model time step in this case), and start/end dates  <br><br>Default = FALSE. |
...
| MEASURE_H         | float     | m                         | Height of humidity measurement                                                                                                                                                                                                                                                                                                        |
