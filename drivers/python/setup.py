...
if ctypesgen:
    ...
    print('running ctypesgen to generate ctypes bindings to vic_run and '
          'drivers/shared: {0}'.format(ctypes_binding_fname))
    ...
    # Put location independent path to shared object into ctypes bindings
    replace(ctypes_binding_fpath,
            '_libs["{0}"] = load_library("{0}")'.format(ext_obj),
            '_libs["{0}"] = load_library(os.path.join(os.path.dirname(__file__), os.pardir, "{0}"))'.format(ext_obj))
...
setup(name='vic',
      ...
      packages=['vic'],
      py_modules=['vic', 'vic.vic', 'vic.driver'],
