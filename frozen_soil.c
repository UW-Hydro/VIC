...
/******************************************************************
  ...
  4-24-07 Removed setup_frozen_soil function (was above this one).  JCA
  4-24-07 Added functionality for EXP_TRANS option.  JCA
...
{    
  ...
  By Ming Pan, mpan@Princeton.EDU
   ...
   Modifications:
   Aug 8, 2006: Integrated with 4.1.0 (from 4.0.3)  JCA
   Aug 8, 2006: Added NOFLUX option.  JCA
   Aug 8, 2006: Added EXP_TRANS option - this allows the heat flux equations
...
int calc_soil_thermal_fluxes(int     Nnodes,
  ...
  /**********************************************************************
   Modifications:
   Aug 24, 2007: Added EXP_TRANS option.  JCA
...
void fda_heat_eqn(double T_2[], double res[], int n, int init, ...)
{
  /**********************************************************************
   ...
   Modifications:
   Aug 8, 2006: Integrated with 4.1.0 (from 4.0.3)  JCA
   Aug 8, 2006: replaced second term of heat flux with an alternate form.  JCA
   Aug 8, 2006: Added NOFLUX option.  JCA
   Aug 8, 2006: Added EXP_TRANS option - this allows the heat flux equations
      to be solved on a transformed grid using an exponential distribution of
      node depths.  JCA
   Aug 9, 2006: Included terms needed for Cs and kappa nodal updating for new
      ice.  JCA
  ...
  **********************************************************************/
  ...
  // calculate residuals if init==0
	...
	if(!EXP_TRANS) {
	  ...
	  flux_term2 = kappa_new[i+1]*((DT_down[i]-DT_up[i])/(Bexp*(Zsum[i+1]+1.))-DT[i]/2./(Bexp*(Zsum[i+1]+1.)*(Zsum[i+1]+1.)));
	}
