...
**********************************************************************/
  ...
  double snow_flux;	/* thermal flux out of snow pack (W/m^2) */
  double C1, C2, C3;
  double error;
  double Tmean;
  ...
  *T1 = (kappa1/2./D1/D2*Tmean + C1/delta_t*T1_old 
  ...
  if((Tmean+ *T1)/2.<0.) {
    ice = moist - maximum_unfrozen_water((Tmean+ *T1)/2.,max_moist,bubble,expt);
    if(ice<0.) ice=0.;
...
/*****
  error = 0.5*(kappa1/D1*(Tmean- *T1)+kappa2/D1*(1.-exp(-D1/dp))*( *T1-T2))
        + D1*Cs1/2./delta_t*(Tmean-Ts_old+ *T1-T1_old)
        - D1*ice_density*Lf/delta_t*(ice-ice0) + snow_flux;
*****/
