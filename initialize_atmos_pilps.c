...
**********************************************************************/
{
  ...
  *************************************************/
  ...
  if(param_set.FORCE_DT[param_set.TYPE[PREC].SUPPLIED-1] == 24) {
	...
	atmos[rec].prec[i] = forcing_data[PREC][idx];
        atmos[rec].prec[i] *= SECPHOUR* (float)global_param.dt;
  ...
  if (!param_set.TYPE[WIND].SUPPLIED) {
	...
	for(i = 0; i < NF; i++) {
	  if(forcing_data[WIND][idx] <options.MIN_WIND_SPEED)
	    ...
	    atmos[rec].wind[i] = sqrt(forcing_data[WIND][idx]*forcing_data[WIND][idx] + 
	    forcing_data[WINDE][idx]*forcing_data[WINDE][idx]);
  ...
  ********************************************************/
  ...
  if(!param_set.TYPE[DENSITY].SUPPLIED) {
    for (rec = 0; rec < global_param.nrecs; rec++) {
      atmos[rec].density[NR] = 3.486*(atmos[rec].pressure[NR]/1000.)/
	(275.0 + atmos[rec].air_temp[NR]);
      for (i = 0; i < NF; i++) {
	atmos[rec].density[i] = 3.486*(atmos[rec].pressure[i]/1000.)/
