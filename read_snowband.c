...
**********************************************************************/
{
  ...
  *AboveTreeLine = (char *)calloc(Nbands,sizeof(char));
  ...
  if ( Nbands > 1 ) {
    ...
    if ( feof(snowband) ) {
      fprintf(stderr, "WARNING: Cannot find current gridcell (%i) in snow elevation band file; setting cell to have one elevation band.\n",
              gridcell);
    ...
    for( band = 0; band < Nbands; band++ ) {
      ...
      if(area_fract<0) {
	sprintf(ErrStr,"Negative snow elevation band area fraction (%f) read from file", 
		area_fract);
	nrerror(ErrStr);
      }
    ...
    if ( total != 1. ) {
      fprintf(stderr,"WARNING: Sum of the snow elevation band area fractions does not equal 1 (%f), dividing each fraction by the sum\n",
	      total);
    ...
    for ( band = 0; band < Nbands; band++ ) {
      ...
      if ( band_elev < 0 ) {
	fprintf(stderr,"Negative snow elevation band elevation (%f) read from file\n", 
		band_elev);
      }
    ...
    for ( band = 0; band < options.SNOW_BAND; band++ ) {
      ...
      if(prec_frac<0) {
	sprintf(ErrStr,"Snow elevation band precipitation fraction (%f) must be between 0 and 1", 
		prec_frac);
	nrerror(ErrStr);
      }
      if ( prec_frac > 0 && (*AreaFract)[band] == 0 ) {
	sprintf(ErrStr,"Snow elevation band precipitation fraction (%f) should be 0 when the area fraction is 0. (band = %d)", 
		prec_frac, band);
	nrerror(ErrStr);
      }
    ...
    if ( total != 1. ) {
      fprintf(stderr,"WARNING: Sum of the snow elevation band precipitation fractions does not equal %d (%f), dividing each fraction by the sum\n",
	      1, total);
      for(band = 0; band < options.SNOW_BAND; band++) 
	(*Pfactor)[band] /= total;
    }
