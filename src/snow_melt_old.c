               ...
               soil_con_struct  *soil_con)
{
        ...
        /*No Snowpack present, handle vapor fluxes on glacier surface */
          if(GlacierIce > 0.0){
            if (GlacierIce < -(snow->vapor_flux)) {
              ...
              snow->blowing_flux *= -(GlacierIce) / snow->vapor_flux;
              snow->vapor_flux = -GlacierIce;
              snow->surface_flux  = -GlacierIce - snow->blowing_flux;
  ...
  /* Mass balance test */
  printf("end\n");
  MassBalanceError = (InitialSwq - snow->swq) + (InitialIwq - snow->iwq) + (RainFall + SnowFall) - melt[0] + snow->vapor_flux + snow->blowing_flux; 
  if(fabs(MassBalanceError) > 1e-6){
    printf("DeltaSnow\tDeltaIwq\tInputs\tmelt\tvapor_flux\tblowing_flux\tMassBallanceError\n");
