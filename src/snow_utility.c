...
// Tavg = (snow->surf_temp+Tgrnd)/2.0+KELVIN;
    ...
    if (options.SNOW_DENSITY == DENS_SNTHRM) {
        ...
        swq = new_snow / (double)MMPERMETER + f * sswq;
        ...
        if (new_snow > 0) {
            if (depth > 0.) {
                ...
                swq += new_snow / (double)MMPERMETER;
                density = (double)h20_density * swq / depth;
            }
            else {
                /* no snowpack present, so snow density equals that of new snow */
                density = density_new;
                swq += new_snow / (double)MMPERMETER;
                depth = calc_snow_depth(swq, density);
            }
        }
        else {
            density = (double)h20_density * swq / snow->depth;
        }
...
**********************************************************************/
    ...
    if (options.SNOW_DENSITY == DENS_SNTHRM) {
        ...
        if (air_temp > 0) {
            density_new = (double)NEW_SNOW_DENSITY + (double)h20_density *
    ...
    /** Aged Snow **/
        ...
        if (cold_content < 0.0 && !MELTING) {
            albedo = NEW_SNOW_ALB * pow(SNOW_ALB_ACCUM_A,
                                        pow((double)last_snow * dt / HOURSPERDAY,
                                            SNOW_ALB_ACCUM_B));
        }
        /* Melt Season */
        else {
...
double
calc_snow_depth(double swq,
                double density)
{
/**********************************************************************
   calc_snow_depth       Joe Hamman        August 28, 2014

   This subroutine computes the snow pack depth.  The simple calculation is
   calculated in a number of places within VIC and has been moved to a function
   to improve consitancy and reusability.
**********************************************************************/
...
double
calc_cold_content(double wq,
                  double temp)
{
/**********************************************************************
   calc_cold_content       Joe Hamman        August 28, 2014
