 ...
 * LAST-MOD: Thu Jul  1 15:38:50 1999 by Keith Aric Cherkauer <cherkaue@u.washington.edu>
 * DESCRIPTION:  
 * DESCRIP-END.
 * FUNCTIONS:    
 * COMMENTS:     
...
float exist_file(float FR8[MAX][3],float lat, float lon, int cells)
{
  ...
  for(i=0;i<cells;i++) 
    {
    if(  ((long)(1000*lat) == (long)(1000*FR8[i][0])) &&  
         ((long)(1000*lon) == (long)(1000*FR8[i][1])) )
      {
