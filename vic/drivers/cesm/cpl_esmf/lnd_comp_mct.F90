...
!
! !DESCRIPTION:
!     run method for the VIC lnd model
!
! !REVISION HISTORY:
!
! !INTERFACE: ------------------------------------------------------------------
...
! !INPUT/OUTPUT PARAMETERS:
    ...
    type(seq_cdata)             ,intent(inout) :: cdata
    type(mct_aVect)             ,intent(inout) :: x2d        ! driver -> vic
    type(mct_aVect)             ,intent(inout) :: d2x        ! vic   -> driver
    type(seq_cdata)             ,intent(inout) :: cdata_s
    type(mct_aVect)             ,intent(inout) :: x2s        ! driver -> vic
    type(mct_aVect)             ,intent(inout) :: s2x        ! vic   -> driver
...
!
! !DESCRIPTION:
!     finalize method for the VIC model
!
! !REVISION HISTORY:
!
! !INTERFACE: ------------------------------------------------------------------
