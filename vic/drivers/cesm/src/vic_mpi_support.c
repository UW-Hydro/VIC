                           ...
                           double *var)
{
    ...
    if (mpi_rank == 0) {
        ...
        dvar = malloc(sizeof(*dvar) * grid_size);
        if (dvar == NULL) {
        ...
        dvar_gathered = malloc(sizeof(*dvar_gathered) * global_domain.ncells);
        if (dvar_gathered == NULL) {
            log_err("Memory allocation error in gather_put_nc_field_double().");
        }

        dvar_remapped = malloc(sizeof(*dvar_remapped) * global_domain.ncells);
                        ...
                        int    *var)
{
    ...
    if (mpi_rank == 0) {
        grid_size = global_domain.n_nx * global_domain.n_ny;
        ivar = malloc(sizeof(*ivar) * grid_size);
        if (ivar == NULL) {
