    ...
    while (!feof(gp)) {
        if (cmdstr[0] != '#' && cmdstr[0] != '\n' && cmdstr[0] != '\0') {
            ...
            *************************************/
                ...
                log_err("MAX_SNOW_TEMP has been deprecated. To specify a maximum"
                        "snow temperature, use the option SNOW_MAX_SNOW_TEMP in"
                        "the vic constants file.");
            }
            else if (strcasecmp("MIN_RAIN_TEMP", optstr) == 0) {
                log_err("MIN_RAIN_TEMP has been deprecated. To specify a"
                        "minimum rain temperature, use the option"
