...
void convolve_new(const size_t nsources,               /*scalar - number of sources*/
...
//            ring[rind] += unit_hydrograph[uhind];
           ...
           ring[rind] += (unit_hydrograph[uhind] * (out_data[rout.rout_param.source_VIC_index[s]][OUT_RUNOFF].data[0] + out_data[rout.rout_param.source_VIC_index[s]][OUT_BASEFLOW].data[0]));
       }
   }
}
